...
item_of() { echo "$J" | sed 's/},{"notificationId"/}\n{"notificationId"/g' | grep "\"notificationId\":$1,"; }
...
check "최신 MESSAGE_RECEIVED, type=MESSAGE, target S11/chatRoomId, content=미리보기" "" "$([ "$CODE" = 200 ] && [ "$(field eventCode)" = MESSAGE_RECEIVED ] && [ "$(field type)" = MESSAGE ] && has "\"target\":{\"screen\":\"S11\",\"matchId\":null,\"chatRoomId\":$ROOM_ID," && has '"content":"안녕하세요 반가워요"' && echo true)"
...
step "8. A 개별 읽음 (MATCH_LIKED) → readAt, unreadCount 2"; req PUT "$NOTI/$LIKED_ID/read" "$TOK_A"; READ_AT=$(field readAt)
check "200, notificationId 일치, readAt 존재, unreadCount=2" "" "$([ "$CODE" = 200 ] && [ "$(field notificationId)" = "$LIKED_ID" ] && [ -n "$READ_AT" ] && [ "$READ_AT" != null ] && [ "$(field unreadCount)" = 2 ] && echo true)"
step "9. 같은 알림 재읽음 → 멱등 (readAt 초 단위 동일)"; req PUT "$NOTI/$LIKED_ID/read" "$TOK_A"
check "200, readAt 그대로, unreadCount=2" "" "$([ "$CODE" = 200 ] && [ "${READ_AT:0:19}" = "$(field readAt | cut -c1-19)" ] && [ "$(field unreadCount)" = 2 ] && echo true)"
step "10. unreadOnly 2건, 전체 3건(읽은 항목 read=true·readDate)"; req GET "$NOTI?page=0&size=20&unreadOnly=true" "$TOK_A"; N=$(items_n); req GET "$NOTI?page=0&size=20" "$TOK_A"
check "2건 / 3건, MATCH_LIKED read=true" "" "$([ "$N" = 2 ] && [ "$(items_n)" = 3 ] && item_of "$LIKED_ID" | grep -q '"eventCode":"MATCH_LIKED"' && item_of "$LIKED_ID" | grep -q '"read":true' && ! item_of "$LIKED_ID" | grep -q '"readDate":null' && echo true)"
...
step "12. 대화량 미충족 상태의 다음 단계 동의 → 거부, 알림 없음 (REVEAL_REQUESTED 는 상호동의 단계에서만)"; req POST "/api/v1/matches/$MATCH_ID/reveal/consent" "$TOK_B" '{"revealLevel":1,"consent":true}'; C1=$CODE; RC=$(field code); req GET "$NOTI?page=0&size=20" "$TOK_A"
check "동의 409 REVEAL_002, A 최신 알림은 여전히 MESSAGE_RECEIVED, 3건" "" "$([ "$C1" = 409 ] && [ "$RC" = REVEAL_002 ] && [ "$(field eventCode)" = MESSAGE_RECEIVED ] && [ "$(items_n)" = 3 ] && echo true)"
...
step "14. A 안읽음 3 (CREATED·MESSAGE·ENDED)"; req GET $NOTI/unread-count "$TOK_A"
check "unreadCount=3" "" "$([ "$CODE" = 200 ] && [ "$(field unreadCount)" = 3 ] && echo true)"
step "15. A 전체 읽음 (S7-08) → updatedCount 3, unreadCount 0"; req PUT $NOTI/read-all "$TOK_A"
check "updatedCount=3, unreadCount=0" "" "$([ "$CODE" = 200 ] && [ "$(field updatedCount)" = 3 ] && [ "$(field unreadCount)" = 0 ] && echo true)"
...
step "18. 페이지: size=2 → 2건/총 4/2페이지, 마지막 페이지 2건 last=true"; req GET "$NOTI?page=0&size=2" "$TOK_A"; P0=$([ "$(items_n)" = 2 ] && [ "$(field totalElements)" = 4 ] && [ "$(field totalPages)" = 2 ] && [ "$(field last)" = false ] && echo true); req GET "$NOTI?page=1&size=2" "$TOK_A"
check "page0: 2건·4·2·last=false, page1: 2건·last=true" "" "$([ "$P0" = true ] && [ "$(items_n)" = 2 ] && [ "$(field last)" = true ] && echo true)"

...
mk() { local n=$1 g=$2 y=$3 r=$4 p=$5; local e="bma66-$n-$TS@example.com"
  ...
  req PUT /api/v1/users/me/preference "$t" "$p"; }
now_iso() { date +%s; }

step "1. 계정 준비: A(남,31,강남,선호 여) C(남,36,강남,선호 여) → 대기 상태 없음(NONE)"
mk A M 1995 SEOUL_GANGNAM '{"preferredGenderCode":"F"}'; mk C M 1990 SEOUL_GANGNAM '{"preferredGenderCode":"F"}'
req GET $Q "$TOK_A"
check "200 status=NONE, queueId=null" "" "$([ "$CODE" = 200 ] && [ "$(field status)" = NONE ] && has '"queueId":null' && echo true)"
...
mk D F 1996 SEOUL_JUNG '{"preferredGenderCode":"M","maxAge":34}'; req POST $Q "$TOK_D"; MATCH1=$(field matchId); ROOM1=$(field chatRoomId)
...
mk B F 1997 SEOUL_JUNG '{"preferredGenderCode":"M"}'; req POST "/api/v1/users/$USER_C/block" "$TOK_B" '{"reason":"test"}'; C1=$CODE; req POST $Q "$TOK_B"; QB=$(field queueId)
check "차단 200, B WAITING" "" "$([ "$C1" = 200 ] && [ "$CODE" = 200 ] && [ "$(field status)" = WAITING ] && echo true)"

step "9. 지역 조건: E(여, 부산, 선호 남 + 서울 전체) 진입 → 강남(서울 하위)의 C 와 성사 (B 는 여라 제외)"
mk E F 1998 BUSAN_HAEUNDAE '{"preferredGenderCode":"M","preferredRegionCode":"SEOUL"}'; req POST $Q "$TOK_E"
...
mk F M 1999 SEOUL_JUNG '{"preferredGenderCode":"F","matchingEnabled":false}'; req POST $Q "$TOK_F"; C1=$CODE; R1=$(field code)
req PUT /api/v1/users/me/preference "$TOK_F" '{"preferredGenderCode":"F","matchingEnabled":true}'; req POST $Q "$TOK_F"
...
mk G M 1994 SEOUL_GANGNAM '{"preferredGenderCode":"F"}'; req POST $Q "$TOK_G"; QG=$(field queueId); req POST $Q "$TOK_G"; QG2=$(field queueId); req DELETE $Q "$TOK_G"; C1=$CODE; req GET $Q "$TOK_G"
...
if [ -n "$DB_EXEC_CMD" ]; then
  ...
  mk H M 1993 SEOUL_GANGNAM '{"preferredGenderCode":"F"}'; for i in 1 2 3; do req POST $Q "$TOK_H"; req DELETE $Q "$TOK_H"; done
...
mk K F 1995 SEOUL_GANGNAM '{"preferredGenderCode":"M"}'; req POST $Q "$TOK_K"; SK=$(field status)

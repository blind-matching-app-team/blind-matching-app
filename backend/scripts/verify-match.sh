...
check "reveal: 0단계 실루엣, 다음 1단계, 0/20 메시지, progressRate 0" "" "$([ "$(field currentLevel)" = 0 ] && [ "$(field currentLevelName)" = 실루엣 ] && [ "$(field nextLevel)" = 1 ] && [ "$(field messageCount)" = 0 ] && [ "$(field requiredMessageCount)" = 20 ] && [ "$(field progressRate)" = 0 ] && echo true)"
...
check "'매칭이 종료됐어요' 포함, 상대 닉네임 미포함" "" "$([ "$CODE" = 200 ] && has '매칭이 종료됐어요' && ! has "$NICK_B" && echo true)"
step "19. B 같은 매칭 다시 그만두기 - 멱등"; req DELETE "/api/v1/matches/$MATCH_ID" "$TOK_B"; req GET "/api/v1/notifications?page=0&size=20" "$TOK_A"
check "200, A 종료 알림은 여전히 1건" "" "$([ "$CODE" = 200 ] && [ "$(echo "$J" | grep -o '매칭이 종료됐어요' | wc -l | tr -d ' ')" = 1 ] && echo true)"

...
개발자센터에서 테스트 키를 발급받는 것만으로 빌링 API 가 열리는지는 문서가 명확히 구분해주지 않는다.

**그래서 이 검증을 먼저 한다.** 막혀 있다면 계약 절차가 선행돼야 하고,
그 사실을 결제 로직을 다 만든 뒤가 아니라 지금 알아야 한다.
...
# 개발자센터 > 테스트 > 테스트 카드 정보에서 확인한다.
# 테스트 환경은 카드번호 앞 여섯 자리(BIN)만 유효하면 등록된다.
TOSS_TEST_CARD_NUMBER=
...
카드번호는 문서에 고정 값이 없으므로 **개발자센터의 테스트 카드 정보를 그대로** 넣는다.
...
- `X-Toss-Topaz` — 에러 상황 시뮬레이션. `test_sk` 키에서만 동작한다. 실패 케이스 검증에 쓴다.

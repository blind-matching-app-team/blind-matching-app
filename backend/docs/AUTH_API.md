...
| `AUTH_003` | 같은 이메일이 다른 수단으로 이미 가입됨. **`&provider=` 가 함께 붙는다** |
| `AUTH_009` / `AUTH_007` | 정지 계정 / 이용 불가 계정 |

**이메일 중복일 때만 가입 수단이 함께 온다.**

```
{failureRedirect}?error=AUTH_003&provider=KAKAO
```

프론트는 이 값으로 "카카오로 가입된 이메일이에요"처럼 안내한다.
`provider` 값은 `LOCAL` / `KAKAO` / `NAVER` / `GOOGLE` 이다.

> **정지 계정(`AUTH_009`)은 상세가 실리지 않는다.** 정지 사유는 자유 텍스트라 쿼리
> 파라미터로 넘기기에 적절하지 않다. 이 경우 프론트는 **이메일 로그인으로 유도**하고,
> 거기서 `data.errorCode=ACCOUNT_SUSPENDED` 와 전체 상세를 받아 이용정지 화면을 띄운다.
> (소셜 경로에서도 전체 상세가 필요하다면 실패용 티켓 발급 구조가 추가로 필요하다 — 9절 참고)
...
- 소셜 로그인 콘솔 등록 및 키 발급.
- **소셜 콜백에서 정지 계정을 만났을 때** 전체 상세를 넘길지.
  현재는 `?error=AUTH_009` 만 가고 프론트가 이메일 로그인으로 유도하는 것을 전제한다.
  소셜 경로에서도 이용정지 화면을 바로 띄우려면 실패 정보를 담을 저장소(실패용 티켓)가
  필요하다. `US_USER_TOKEN` 은 `USER_ID` 가 NOT NULL 이라 사용자 없는 실패는 담지 못한다.

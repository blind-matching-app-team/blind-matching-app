...
| `REVEAL_REQUESTED` | **S7-12** | ???님이 다음 단계를 요청했어요 | 상호동의 단계(2단계)에서 상대가 먼저 동의 | S10 (`matchId`), 동의 모달 자동 오픈 | 발행 중 — 단, 대화량 조건(50건/30분) 충족 후에만 발생해 REST 검증에서는 실측 불가. BMA-69(24h+10건+상호동의) 반영 시 조건만 바뀌고 코드는 그대로 |
